class FindFlagwarJob < ApplicationJob
  ...
  def query(options = {})
    ...
    @@DISCUSSIONS_CACHE[tag] = by_cashout.map do |comment|
      ...
      next if base_value(comment.pending_payout_value) < 0.001 # no author payout, don't care
    ...
    if !!tag
      ...
      tags.each do |t|
        ...
        begin
          ...
          by_cashout += response.result
          Rails.logger.info("Got discussions for tag \"#{t}\": #{result_size}, (uniqe total: #{by_cashout.uniq.size})")
        rescue => e
